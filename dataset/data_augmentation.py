...
def defor_3D_bb(pc, R, t, s, nocs, model, sym=None, aug_bb=None):
    ...
    if sym[0] == 1:  # y axis symmetry
        ...
        pc_new = torch.mm(R, pc_reproj.T) + t.view(3, 1)
        pc_new = pc_new.T
    return pc_new, s, nocs, model ,nocs_scale_aug
...
def defor_3D_bc(pc, R, t, s, model_point, nocs_scale, nocs):
    ...
    return pc_new, size_new, model_point, nocs,nocs_scale_aug
...
def defor_3D_pc(pc, r):
    points_defor = torch.clip(r*torch.randn(pc.shape).to(pc.device),min=-0.005,max=0.005)
    # points_defor=0.01*torch.randn(pc.shape).to(pc.device)
    # points_defor=torch.clamp(points_defor,min=-(pc*0.02),max=(pc*0.02))
    pc = pc + points_defor
    return pc
